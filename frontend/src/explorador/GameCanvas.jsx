  ...
  const CFG = {
    ...
    enemyAttackRange: 42, // distância para iniciar o pulo (em px)
    enemyAttackDamage: 10, // dano único do ataque
    enemyAttackJumpTime: 0.22, // duração do "pulo" em segundos
    enemyAttackRecoverTime: 0.35, // tempo parado após o pulo, em segundos
    enemyAttackCooldown: 0.9, // tempo mínimo entre ataques, em segundos
    enemyAttackHitRadius: 26, // raio para considerar que acertou o player
    energyPickup: 8,
    healPickup: 6,
  ...
  */
  ...
  export default function ExplorerGameCanvas({
    ...
    const enemyDashParticlesRef = useRef([]); // partículas do pulo dos inimigos
    ...
    useEffect(() => {
      ...
      world.current.enemies = built.enemies.map((e) => {
        ...
        return {
          ...
          __lastHeardAt: 0,
          // >>> CAMPOS DO ATAQUE <<<
          __attack: null, // dados do ataque atual (pulo)
          __nextAttackAt: 0, // timestamp (ms) para próximo ataque permitido
        };
      });
    ...
    function update(dt) {
      ...
      for (const e of world.current.enemies) {
        ...
        const isInMeleeAttack = e.state === "attack" && e.__attack;
        ...
        if (!isInMeleeAttack) {
          ...
          const canHear = noiseFactor > 0 && dist < noiseRadius * noiseFactor;
          ...
          if (sees) {
            ...
            // Sentinela e patrulheiro em guard pode sair para investigar um som
            if (canHear) {
              const now = performance.now();
              e.__lastHeardAt = now;
              e.__investigate = { x: p.x, y: p.y, until: now + 3500 }; // 3.5s de interesse

              // qualquer inimigo em patrulha/guarda passa a "return" para investigar
              if (e.state === "patrol" || e.state === "guard") {
                e.state = "return";
                e.__path = planPathForEnemy(e, p.x, p.y);
                e.__detourT = 0;
                e.__detourDir = null;
                e.__wpBlockedT = 0;
              }
        ...
        switch (e.state) {
          ...
          case "chase": {
            ...
            if (!seesNow) {
              // (o que você já fazia para transicionar para search)
            ...
            // >>> VERIFICA SE PODE INICIAR ATAQUE CORPO-A-CORPO <<<
            const nowMs = performance.now();
            const canAttackNow =
              dist <= CFG.enemyAttackRange &&
              nowMs >= (e.__nextAttackAt || 0) &&
              hasLineOfSight(e.x, e.y, p.x, p.y, world.current.walls);
            ...
            if (canAttackNow) {
              ...
              e.__attack = {
                phase: "jump",
                t: 0,
                duration: CFG.enemyAttackJumpTime,
                recover: CFG.enemyAttackRecoverTime,
                fromX: e.x,
                fromY: e.y,
                toX: p.x, // tranca a posição alvo no início do pulo
                toY: p.y,
                didDamage: false,
              };
              e.__nextAttackAt = nowMs + CFG.enemyAttackCooldown * 1000;
              break; // não faz movimento normal neste frame
            }

            // >>> movimento normal de chase (igual você já tinha) <<<
          ...
          case "attack": {
            ...
            if (atk.phase === "jump") {
              // interpolação do pulo
              const s = Math.min(1, atk.t / atk.duration);
              const nx = atk.fromX + (atk.toX - atk.fromX) * s;
              const ny = atk.fromY + (atk.toY - atk.fromY) * s;
              e.x = nx;
              e.y = ny;
              e.dir = Math.atan2(atk.toY - atk.fromY, atk.toX - atk.fromX);

              // >>> TRAIL DE PARTÍCULAS DO PULO <<<
              {
                const N = 2; // quantas partículas por frame (ajuste se quiser mais)
                for (let i = 0; i < N; i++) {
                  const ang = e.dir + (Math.random() * 0.7 - 0.35); // espalha um pouco
                  const spd = 60 + Math.random() * 90;
                  enemyDashParticlesRef.current.push({
                    x: e.x + (Math.random() * 6 - 3), // leve jitter
                    y: e.y + (Math.random() * 6 - 3),
                    vx: -Math.cos(ang) * spd * 0.7, // vai "para trás" do movimento
                    vy: -Math.sin(ang) * spd * 0.7,
                    life: 0.25 + Math.random() * 0.15, // duração curta
                    age: 0,
                    r: 2 + Math.random() * 2, // raio
                  });
                }
              }
              // <<< FIM TRAIL >>>

              // aplica dano UMA vez, mais ou menos no meio do pulo
              if (!atk.didDamage && s >= 0.55) {
                const dToPlayer = Math.hypot(
                  playerRef.current.x - e.x,
                  playerRef.current.y - e.y
                );
                if (dToPlayer <= CFG.enemyAttackHitRadius) {
                  setHud((h) => ({
                    ...h,
                    hp: Math.max(0, h.hp - CFG.enemyAttackDamage),
                  }));
                  atk.didDamage = true;
                }
              }

              // fim do pulo -> entra em recover
              if (s >= 1) {
                atk.phase = "recover";
                atk.t = 0;
              }
            } else if (atk.phase === "recover") {
              // inimigo fica parado um tempo
              if (atk.t >= atk.recover) {
                e.__attack = null;
                e.state = "chase"; // volta a perseguir
              }
            }
        ...
        if (
          e.state !== "attack" && // dano de contato só fora do ataque "pulo"
          dist < 26 &&
          hasLineOfSight(e.x, e.y, p.x, p.y, world.current.walls)
        ) {
          setHud((h) => ({
            ...h,
            hp: Math.max(0, h.hp - enemyDpsEff * dt),
          }));
        }
      }

      // === partículas do dash dos inimigos (update) ===
      if (enemyDashParticlesRef.current.length) {
        enemyDashParticlesRef.current = enemyDashParticlesRef.current.filter(
          (p) => {
            p.age += dt;
            if (p.age >= p.life) return false;

            // levemente "freando" e caindo
            const t = p.age / p.life;
            const drag = 1 - t * 0.6; // desacelera no fim
            p.vx *= drag;
            p.vy = p.vy * drag + 40 * dt; // um pouco de gravidade
            p.x += p.vx * dt;
            p.y += p.vy * dt;
    ...
    function draw() {
      ...
      for (const e of world.current.enemies) {
        ...
        // Partículas do dash dos inimigos
        if (enemyDashParticlesRef.current.length) {
          for (const pz of enemyDashParticlesRef.current) {
            const t = Math.max(0, Math.min(1, pz.age / pz.life)); // 0..1
            const alpha = 0.9 * (1 - t); // some no fim
            ctx.beginPath();
            ctx.arc(pz.x, pz.y, pz.r, 0, Math.PI * 2);
            ctx.fillStyle = `rgba(248,250,252,${alpha})`; // quase branco
            ctx.fill();
          }
        }
